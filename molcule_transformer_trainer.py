...
class mtdti_trainer():
    ...
    def train(self, remain_epochs, log='stdout'):
        ...
        for batch in self.train_batch:
            ...
            loss = self.criterion(predicts.reshape(-1,n_tokens), targets.view(-1))
            loss.backward()
            self.optimizer.step()
            ...
            if i % log_interval == 0 and i > 0:
                ...
                for j in range(len(batch)):
                    for k in range(len(batch.output[j])):
                        if batch.output[j][k] in self.untargeted_tokens:
                            continue
                ...
                log_str = '| epoch {:3d} | {:5d}/{:5d} batches | lr {:02.10f} '
                          '| ms/batch {:5.2f} | loss {:5.8f} | acc {:6.4f}'.format(
                            epoch, i, len(self.train_batch), self.decay_scheduler.get_last_lr()[0],
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for batch in test_batch:
                data, targets = batch.input, batch.output
                predicts = self.model(data).transpose(0,1)
                total_loss += len(data) * self.criterion(predicts.reshape(-1, n_tokens), targets.view(-1)).item()
    ...
    def count_hit(h, Y, idx):
        """
        h : (N, S, Word_dim) predicted results from transformer should be transposed.
